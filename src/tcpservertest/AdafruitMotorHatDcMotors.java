...
public class AdafruitMotorHatDcMotors extends AdafruitMotorHatBase {
  ...
  protected class DcMotorConfiguration {
    ...
    // Read only accessors
    public byte PwmPinPower() {
      return PwmPinPower;
    }

    public byte PwmPinDirection1() {
      return PwmPinDirection1;
    }

    public byte PwmPinDirection2() {
      return PwmPinDirection2;
    }
  }
  ...
  public enum EMotorCommands {
    Foreward, Backward, Release
  }
  ...
  public void DriveMotor(EMotorCommands[] pMotorCommands, byte[] pMotorSpeeds) {
    try {
      ...
      for (int ij = 0; ij < pMotorCommands.length; ij++) {
        // Set motor direction before applying speed set points
        System.out.println("Setting Motor " + ij + " Direction to " + pMotorCommands[ij]);
      ...
      for (int ij = 0; ij < pMotorCommands.length; ij++) {
        // Set motor speed
        System.out.println("Setting Motor " + ij + " Speed to " + pMotorSpeeds[ij]);
